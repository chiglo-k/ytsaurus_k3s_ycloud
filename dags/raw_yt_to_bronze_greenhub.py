...
DEFAULT_RAW_PATH = "//home/raw/greenhub"
DEFAULT_BRONZE_ROOT = "//home/bronze_stage/greenhub"
...
def main() -> int:
    ...
    parser.add_argument("--source-name", default="greenhub")
    parser.add_argument("--file-hash", default="")
    args = parser.parse_args()

    spark = (
        SparkSession.builder
        .appName(f"raw_yt_to_bronze_greenhub_{args.part_index}")
        .getOrCreate()
    )

    raw_yt = yt_table_path(args.raw_path)
    print(f"[INFO] Reading raw YT table: {raw_yt}", flush=True)

    df_raw = spark.read.format("yt").load(raw_yt)
    ...
    source_hash_base = f"{args.raw_path}|{args.batch_id}|{rows_in}"
    file_hash = args.file_hash or hashlib.sha256(source_hash_base.encode("utf-8")).hexdigest()
    ...
    df_with_uids = (
        ...
        .withColumn("_source_file", F.lit(args.raw_path))
    ...
    required_uid_cols = [
        "fact_uid", "device_uid", "country_uid", "timezone_uid",
        "battery_state_uid", "network_status_uid", "charger_uid", "health_uid",
        "network_type_uid", "mobile_network_type_uid", "mobile_data_status_uid",
        "mobile_data_activity_uid", "wifi_status_uid",
    ]
    ...
    print(f"[INFO] fact rows ready: {fact_count:,} -> {yt_table_path(fact_path)}", flush=True)
    ...
    for dim_name, raw_col, namespace in dims_simple:
        ...
        n = df_dim.count()
        print(f"[INFO] dim_{dim_name} ({n} distinct rows) -> {yt_table_path(path)}", flush=True)
    ...
    print(f"[INFO] dim_device ({n_dev} distinct devices) -> {yt_table_path(path)}", flush=True)
    ...
    print(f"[DONE] raw path    : {args.raw_path}", flush=True)
